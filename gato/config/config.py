...
GIA_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "gia-project/gato-80m": "https://huggingface.co/gia-project/gato-80m/blob/main/config.json",
    "gia-project/gato-387m": "https://huggingface.co/gia-project/gato-387m/blob/main/config.json",
    "gia-project/gato-1.27b": "https://huggingface.co/gia-project/gato-1.27b/blob/main/config.json",
}


class GatoConfig(GPTNeoConfig):
    r"""
    This is the configuration class to store the configuration of a [`GatoModel`]. It is used to instantiate an
    GATO model according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of the GATO
    [gia-project/gato-1.27b](https://huggingface.co/gia-project/gato-1.27b) architecture.
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 31_025):
            ...
            the [`inputs_ids`] passed when calling [`GatoModel`].
    ...
    @staticmethod
    def from_args(args: Arguments) -> "GatoConfig":
        config = GatoConfig.from_pretrained("gia-project/gato-1.27b")
        config.patch_size = args.patch_size
        # TODO add other args
        return config


AutoConfig.register("gato", GatoConfig)
