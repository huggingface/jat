...
def load_gia_dataset(
    ...
    Args:
        task_names (str): Name or or list of names of the task to load. See the available tasks
    ...
    for task_name in task_names:
        if task_name not in all_tasks:
            # It is either a prefix (example: "mujoco"), or a dedicated keyword (example: "all").
            if task_name == "all":
    ...
    if task_name.startswith("babyai"):
...
def load_batched_dataset(task_name: str, args: DatasetArguments, split: str = "all") -> DatasetDict:
    ...
    Example:
        ...
        (4074, 28, 27)
    """
    dataset = load_gia_dataset(task_name, split=split, load_from_cache=args.load_from_cache)
