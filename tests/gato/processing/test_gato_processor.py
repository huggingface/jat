...
from gato.processing.processing import GatoImageProcessor, GatoProcessor, GiaContinuousTokenizer, GiaDiscreteTokenizer
...
def test_extract_patches():
    processor = GatoImageProcessor(patch_size=3)
...
def test_gia_processor_padding_max_length_none(features, seq_len):
    processor = GatoProcessor(seq_len=seq_len)
...
def test_gia_processor_local_positions_singleton_single_modality():
    processor = GatoProcessor(local_positions_groups=[["continuous_observations"]])  # only text
...
def test_gia_processor_local_positions_non_singleton_multi_modality():
    processor = GatoProcessor(local_positions_groups=[["discrete_observations", "continuous_observations"]])
...
def test_gia_processor_mask_loss_modalities_episode():
    processor = GatoProcessor(mask_loss_modalities=["continuous_observations"])
    data = generate_data(2, ["discrete_observations", "continuous_observations", "discrete_actions"])
    out = processor(**data, truncation=False, padding=False)
...
def test_gia_processor_mask_loss_modalities_standalone():
    processor = GatoProcessor(mask_loss_modalities=["text"])
